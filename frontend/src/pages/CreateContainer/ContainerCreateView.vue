  ...
  <div class="container-create-view">
    ...
    >
      <n-tab-pane name="basic" tab="基础">
        <BasicTab ref="basicTabRef" v-model="basicForm" />
      </n-tab-pane>
...
const basicTabRef = ref<TabFormExpose | null>(null)
...
const handleCreate = async () => {
  // 校验基础表单
  try {
    await basicTabRef.value?.validate()
  } catch {
  ...
  try {
    ...
    if (response.code !== 0) {
      message.error(response.msg || '容器创建失败')
      return
    }
