    ...
    <n-card :title="container.name" hoverable class="container-card" :class="{ 'card-updating': isUpdating }">
        ...
        <n-space vertical>
            ...
            <div class="container-info">
                <n-descriptions :column="1" size="small">
                    ...
                    <n-descriptions-item label="当前摘要" v-if="container.currentDigest">
                        <n-text code class="digest-text">{{ formatDigest(container.currentDigest) }}</n-text>
                    </n-descriptions-item>

                    <n-descriptions-item label="远程摘要" v-if="container.remoteDigest">
                        <n-text code class="digest-text">{{ formatDigest(container.remoteDigest) }}</n-text>
                    </n-descriptions-item>
            ...
            <div v-if="hasLabels" class="container-labels">
                ...
                <n-space style="margin-top: 4px;">
                    <n-tag v-for="(value, key) in visibleLabels" :key="key" size="tiny" type="info">
                        {{ key }}={{ value }}
                    </n-tag>
        ...
        <template #action>
            <n-space justify="space-between">
                <!-- 基础操作 -->
                <n-button-group>
                    <n-button v-if="!container.running" @click="$emit('start')" type="primary" size="small"
                        :loading="loading">
                        <template #icon>
                            <n-icon>
                                <PlayCircleOutline />
                    ...
                    <n-button v-if="container.status === 'UpdateAvailable' && !container.skipped"
                        @click="$emit('update')" type="info" size="small" :loading="isUpdating">
                        <template #icon>
                            <n-icon>
