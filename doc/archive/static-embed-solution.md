...
func (s *Server) setupStaticRoutes(r *gin.Engine) {
    ...
    if staticDir != "" {
        // 使用外部静态目录
        logger.Logger.Info("使用外部静态资源目录", zap.String("dir", staticDir))
...
# 测试运行（使用嵌入资源）
CONFIG_PATH=/tmp/test ./watch-docker-native

# 测试运行（使用外部资源）
STATIC_DIR=./static CONFIG_PATH=/tmp/test ./watch-docker-native
```
...
STATIC_DIR=./static CONFIG_PATH=/tmp/test ./watch-docker-docker
```
