...
- `GET /api/containers/:id/files` - 列出容器目录内容
- `GET /api/containers/:id/files/content` - 获取文件内容
- `PUT /api/containers/:id/files/content` - 更新文件内容
- `POST /api/containers/:id/files/download-token` - 生成下载令牌
- `GET /api/containers/:id/files/download` - 下载文件/目录（支持临时令牌）
- `POST /api/containers/:id/files/create` - 创建文件/目录
- `POST /api/containers/:id/files/rename` - 重命名文件/目录
- `DELETE /api/containers/:id/files/delete` - 删除文件/目录
- `POST /api/containers/:id/files/chmod` - 修改文件权限
- `POST /api/containers/:id/files/upload` - 上传文件

**终端访问**

- `GET /api/shell/ws` - Shell 终端 WebSocket
- `GET /api/containers/:id/shell/ws` - 容器 Shell WebSocket
...
Watch Docker 提供了强大的容器文件系统可视化管理功能，让你可以像管理本地文件一样管理容器内的文件。

### ✨ 功能特性

#### 📂 文件浏览

- **目录导航** - 可视化浏览容器内的文件系统结构
- **面包屑导航** - 快速跳转到任意父级目录
- **文件类型识别** - 自动识别文件、目录、符号链接
- **详细信息** - 显示文件大小、权限、修改时间
- **隐藏文件** - 支持显示/隐藏以 `.` 开头的文件
...
- **单文件下载** - 直接下载文件到本地
- **目录下载** - 将整个目录打包为 tar 文件下载
- **安全令牌** - 使用临时令牌进行安全下载
- **流式传输** - 支持大文件的流式下载
- **令牌过期** - 下载令牌 60 秒有效期，使用后立即失效
...
```
┌─────────────────────────────────────────────────────────┐
│ ⬆️ 🏠 🔄 👁️ ☁️  /root/app/config                        │ 工具栏
├─────────────────────────────────────────────────────────┤
│ [名称 ⬆️]  [大小 ⇕️]  [权限]  [修改时间 ⇕️]              │ 表头（可排序）
├─────────────────────────────────────────────────────────┤
│ 📂 conf                     -    755   2024-02-03      │
│ 📂 logs                     -    755   2024-02-03      │
│ 📄 app.yaml             1.2KB    644   2024-02-02      │
│ 📄 config.json          856B     644   2024-02-01      │
└─────────────────────────────────────────────────────────┘
```
...
- **Docker Archive API** - 使用 Docker 原生 API 操作文件
- **流式传输** - 大文件采用流式处理
- **临时令牌管理** - 内存中管理下载令牌
- **定期清理** - 自动清理过期令牌
- **权限检查** - 验证文件操作权限
...
- **虚拟列表** - 使用 `virtua/vue` 渲染大量文件
- **Composable 架构** - 模块化的功能组件
- **响应式设计** - 使用 `@vueuse/core` 检测设备类型
- **主题适配** - 完美适配亮色/暗色主题
- **TypeScript** - 完整的类型安全
...
1. **容器状态** - 只有运行中的容器才能进行文件修改
2. **文件大小** - 编辑器适合编辑小型文本文件（< 10MB）
3. **二进制文件** - 不支持编辑二进制文件
4. **符号链接** - 显示链接目标，但操作实际文件
5. **权限要求** - 某些系统文件可能需要特殊权限
6. **并发操作** - 同时编辑同一文件可能导致冲突

### 🔧 配置选项

文件浏览器功能无需特殊配置，自动为所有容器启用。如果需要限制访问：

```yaml
# config.yaml
server:
  readOnly: true # 全局只读模式，禁止所有文件修改
```

### 🎯 使用建议

1. **定期备份** - 在修改重要文件前先下载备份
2. **小步提交** - 一次只修改一个文件，避免大批量操作
3. **权限检查** - 修改系统文件前确认有足够权限
4. **只读容器** - 对重要容器设置只读保护
5. **日志审计** - 定期检查文件操作日志
